...
func initialize():
	...
	step_time = max(right_leg.step_time,left_leg.step_time)
...
func _physics_process(delta: float) -> void:
	...
	if(right_target != null && !stepping && distance_to_right_target > stride_distance && distance_to_right_target > distance_to_left_target):
		step(right_leg,right_target)	
	if(left_target != null && !stepping && distance_to_left_target > stride_distance):
...
func step(leg,target): #Update the foot location to the target with an animation
	...
	t.tween_property(leg.target,"global_position",leg.rest_target.global_position,step_time/2)
	t.tween_property(leg.target,"global_position",target,step_time/2)
...
func look_for_foothold(leg):
	...
	var dir = (leg.global_position-Vector3.UP*min_length) + robot.velocity*1.25*step_time - leg.global_position
	#var query = PhysicsRayQueryParameters3D.create(legs_center.global_position,legs_center.global_position+dir*2,0b001)
	var query = PhysicsRayQueryParameters3D.create(leg.global_position,leg.global_position + 2*dir,0b001)
